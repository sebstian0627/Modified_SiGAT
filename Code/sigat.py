...
NEG_LOSS_RATIO = 1
INTERVAL_PRINT = 20
# DATASET_NUM_DIC[dataset]=116836
NUM_NODE = 3783
...
class SiGAT(nn.Module):
    ...
    def criterion(self, nodes, pos_neighbors, neg_neighbors):
        ...
        # print(pos_neighbors_list)
        # print(neg_neighbors_list)
        ...
        for index, node in enumerate(nodes):
            ...
            # z1 = z1/torch.norm(z1)
            if pos_num > 0:
                # weight = weights_un[(node,list(pos_neighbors[node])[0])]
                # weight_pos = [weights_un[(node,i)] for i in list(pos_neighbors[node])]
                # weight_pos = torch.FloatTensor(weight_pos).to(DEVICES)

                # print(len(weight))
                # print(weight_pos)
                # print(torch.IntTensor(weight_pos))
                # print(torch.FloatTensor(weight_pos))
                pos_neig_embs = nodes_embs[pos_neigs, :]
                # print((F.logsigmoid(torch.einsum("nj,j->n", [pos_neig_embs, z1]))).shape)
                # print(torch.einsum("nj,j->n", [pos_neig_embs, z1]).shape)
                
                # pos_neig_embs = pos_neig_embs/torch.norm(pos_neig_embs,dim = 0)



                # print(z1.shape, pos_neig_embs.shape)
                # loss_pku = -1 * torch.sum(F.logsigmoid(torch.dot(weight_pos,torch.einsum("nj,j->n", [pos_neig_embs, z1]))))
                # loss_pku = -1 * torch.sum(torch.einsum("nj,j->n", [pos_neig_embs, z1])/TEMPERATURE)
                # loss_pku =  - torch.log(torch.sum(torch.exp( torch.einsum("nj,j->n",[pos_neig_embs, z1])/TEMPERATURE)))
            ...
            if C == 0:
                C = 1
            if neg_num > 0:
                # weight_neg = [weights_un[(node,i)] for i in list(neg_neighbors[node])]
                # weight_neg = torch.FloatTensor(weight_neg).to(DEVICES)
                neg_neig_embs = nodes_embs[neg_neigs, :]
                # z1 = z1/torch.norm(z1)
                # neg_neig_embs = neg_neig_embs/torch.norm(neg_neig_embs,dim = 0)
                loss_pku = -1 * torch.sum(F.logsigmoid(-torch.einsum("nj,j->n", [neg_neig_embs, z1])))
                
                # loss_pku =  torch.log(torch.sum(torch.exp( torch.einsum("nj,j->n",[neg_neig_embs, z1])/TEMPERATURE)))
                # print(loss_pku)
                # loss_pku = -1 * torch.sum(F.logsigmoid(torch.dot(weight_neg , torch.einsum("nj,j->n",[neg_neig_embs, z1]))))
...
def load_data2(filename='/content/drive/MyDrive/ELL880_Project/SiGAT/experiment-data/wikic_train1.txt', add_public_foe=True):
    ...
    with open(filename) as fp:
        for i, line in enumerate(fp):
            ...
            person1 = int(float(info[0]))
            person2 = int(float(info[1]))
            v = float(info[2])
            adj_lists3[person2].add(person1)
            adj_lists3[person1].add(person2)
            weights[(person1,person2)] =  v
            weights_un[(person1,person2)] = weights_un.get((person1,person2),0)+v
            weights_un[(person2,person1)] =weights_un.get((person2,person1),0)+v
            if v> 0:
...
def run( dataset='bitcoin_alpha', k=2):
    num_nodes = NUM_NODE + 3

    # adj_lists1, adj_lists2, adj_lists3 = load_data(k, dataset)
    filename = '/content/drive/MyDrive/ELL880_project/final_train_edge.edgelist'
    ...
    def func(adj_list):
        ...
        for a in adj_list:
            for b in adj_list[a]:
                edges.append((a, b))
                weightssss.append(weights_un[(a,b)])
        
        edges = np.array(edges)
        if len(edges) == 0: # fix missing motifs edges 
            edges = np.array([[0, 0]]) 
        # print(len(edges), num_nodes, max(edges[:,0]), max(edges[:,1]))
        adj = sp.csr_matrix((weightssss, (edges[:,0], edges[:,1])), shape=(num_nodes, num_nodes))
        return adj
    ...
    for epoch in range(EPOCHS + 2):
        ...
        if epoch % INTERVAL_PRINT == 0:
            ...
            #change embedding name
            fpath = os.path.join(OUTPUT_DIR, 'embedding-{}-{}-{}_lasttime.npy'.format(dataset, k, str(epoch)) )
    ...
    plt.plot(range(len(lossss)),lossss,label='Training loss')
    plt.xlabel('epochs')
    plt.ylabel('train_loss')
    plt.legend()
    plt.savefig("trainloss_lasttime.png")
    plt.show()
